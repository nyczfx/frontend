...
export default function FFMpegClient() {
  ...
  const handleConvert = async () => {
    ...
    try {
      // Importação dinâmica só no browser
    

      const ffmpeg = createFFmpeg({
        log: true,
        corePath: '/ffmpeg/ffmpeg-core.js', // arquivos na pasta public
      });

      if (!ffmpeg.isLoaded()) {
        console.log('Carregando FFmpeg do navegador...');
        await ffmpeg.load();
      ...
      alert('Erro ao converter o áudio. Veja o console.');
    }
    ...
    <div className="w-full px-10 py-6 text-white">
      ...
      <div className="bg-[#0c0c0c] border border-neutral-900 rounded-2xl p-6 shadow-lg space-y-4">
        <label className="flex flex-col items-center justify-center w-full h-36 bg-neutral-950 border border-neutral-900 rounded-xl cursor-pointer hover:border-neutral-700 transition">
          ...
          <span className="text-neutral-400 hover:text-white transition">Selecionar áudio</span>
