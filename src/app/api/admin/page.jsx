...
export default function Admin() {
  ...
  async function loadUsers() {
    ...
    const todayCount = data.filter(
      u => new Date(u.created_at) >= startToday
    ).length;

    const activeCount = data.filter(
      u => new Date(u.created_at) >= startWeek
    ).length;
    ...
    <div className="p-8 text-white space-y-8">
      ...
      <div>
        ...
        <p className="text-gray-400 mt-1">
          Visão geral dos usuários da plataforma
        </p>
      </div>
      ...
      <div className="bg-[#0f0f0f] border border-[#2a2a2a] rounded-2xl p-6">
        ...
        <div className="space-y-3 max-h-[420px] overflow-y-auto pr-2">
          {users.map(user => (
            <div
              key={user.id}
              className="flex justify-between items-center bg-[#141414] border border-[#2a2a2a] rounded-xl p-4"
            >
              <div>
